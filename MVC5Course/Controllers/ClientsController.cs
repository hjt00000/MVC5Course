...
namespace MVC5Course.Controllers
{
    public class ClientsController : Controller
    {
        ...
        public ActionResult Index(string search,int page = 1)
        {
            ...
            var client = db.Client.Include(c => c.Occupation).OrderBy(p => p.ClientId).AsQueryable();
            if(!string.IsNullOrEmpty(search))
            {
                client = client.Where(p => p.FirstName.Contains(search) || p.LastName.Contains(search));
            }
            ViewBag.search = search;
            var data = client.ToPagedList(page, 10);
            return View(data);
        }
...
";
                ...
                //return View("Index",db.Client.Include(c => c.Occupation).Take(10).ToList());
            }
            ViewBag.OccupationId = new SelectList(db.Occupation, "OccupationId", "OccupationName", client.OccupationId);
            return View(client);
        }
